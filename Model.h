...
public:
	...
	// model-space AABB computed at load time
...
private:
	...
	Mesh processMesh(aiMesh *mesh, const aiScene *scene)
	{
		...
		// compute mesh-local AABB and update model AABB
		...
		// update model-wide AABB
		aabbMin.x = std::min(aabbMin.x, localMin.x);
		aabbMin.y = std::min(aabbMin.y, localMin.y);
		aabbMin.z = std::min(aabbMin.z, localMin.z);
		aabbMax.x = std::max(aabbMax.x, localMax.x);
		aabbMax.y = std::max(aabbMax.y, localMax.y);
		aabbMax.z = std::max(aabbMax.z, localMax.z);
